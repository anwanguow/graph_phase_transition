...
def load_and_process_data(path):
    ...
    sop = np.load(f"{path}/swap/sop.npy", allow_pickle=True)
    sop = (sop - np.min(sop)) / (np.max(sop) - np.min(sop))
    return gaussian_filter1d(surv_prob, 1), gaussian_filter1d(sop, 1)
...
for i, r in enumerate(ratios):
    ax2.plot(T, data[r][1], color=colors[i], label=rf'$\mathcal{{D}}_{{s,{r}}}$')

ax1.set_ylabel(r"$P_{amo}$", labelpad=10)
ax2.set_ylabel(r"$\mathcal{D}_s$", labelpad=10)
...
plt.savefig("../Figure/Fig_5b.png", dpi=300, bbox_inches='tight')
plt.show()
