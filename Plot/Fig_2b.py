...
for i in range(init_number):
    for j in range(11):
        file_name = path + f"simulation/md_{i}/answer/dist_s_{j*100}.npy"
...
for i in range(11):
    plt.plot(T, measures_avg[i], color=colors[i], label=f"$n_{{{i}}}={i*100}$")
...
plt.tight_layout()
plt.savefig("../Figure/Fig_2b.png", dpi=300, bbox_inches='tight')
plt.show()
